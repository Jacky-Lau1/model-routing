...
import { pricingCatalogEvidence } from "./cost.js";
import {
...
import type {
  ...
  ProviderBudgetState, ProviderResponse, QualityGatePolicy, QualityGateReport, RequestBudget, RouteBinding, RouteDecision, TaskPackage,
...
export class RouterCoreService {
  ...
  async execute(taskId: string, approvalSummaryHash: string): Promise<RouterCompactStatus> {
    ...
    try {
      ...
      if (!record.approval_record) {
        ...
        const approval = approveContracts(subject(record), {
          approvalId: `approval-${stableHash({ task_id: taskId, summary: approvalSummaryHash }).slice(0, 24)}`,
          approvedAt,
        ...
        record = { ...record, approval_record: approval, updated_at: approvedAt.toISOString() };
    ...
    try {
      ...
      const execution = await this.executeProvider(record, lease);
  ...
  async status(taskId: string): Promise<RouterCompactStatus> {
    ...
    return {
      ...
      approval_summary: approvalSummary(record),
  ...
  async reviewEvidence(taskId: string): Promise<RouterReviewEvidence> {
    ...
    const bytes = await readEvidenceArtifact(this.store.root, record.evidence_bundle_reference);
  ...
  async repair(taskId: string, evidenceBundleHash: string, approvalSummaryHash: string): Promise<RouterCompactStatus> {
    ...
    if (!hashesEqual(approvalSummary(record).approval_summary_hash, approvalSummaryHash)) { await this.attempts.blockLocalFailure(taskId, record.execution_context.run_id, record.approval_record.approval_hash, "Repair requested a provider, model, budget, scope, privacy, or approval change"); return this.status(taskId); }
    ...
    try {
      ...
      await this.saveRecord(record);
      const execution = await this.executeProvider(record, lease, 1, repairInstruction);
      if (!execution) return this.status(taskId);
      ...
      record = validation.record;
      await this.worktrees.retain(lease);
      if (!validation.bundle.quality_passed) await this.attempts.finalizeReview(taskId, record.execution_context.run_id, record.approval_record!.approval_hash, "BLOCKED", "Quality gate did not pass after the only controlled repair");
      return this.status(taskId);
    } catch (error) {
      await this.attempts.blockLocalFailure(taskId, record.execution_context.run_id, record.approval_record!.approval_hash, error).catch(() => undefined);
      return this.status(taskId);
  ...
  private async executeProvider(record: CoreRecord, lease: WorktreeLease, round = 0, repairInstruction?: string): Promise<CoreExecutionEvidence | undefined> {
    const approval = record.approval_record!;
  ...
  private async validateAndBundle(record: CoreRecord, lease: WorktreeLease, round = 0): Promise<{ record: CoreRecord; bundle: EvidenceBundle }> {
    ...
    const qualityGate = {
      run_id: record.execution_context.run_id, task_id: record.task_package.task_id, base_commit: record.execution_context.base_commit,
      plan_hash: record.task_package.task_package_hash, approval_hash: approval.approval_hash,
      isolation_hash: record.worktree_binding.isolation_hash, worktree_id: record.execution_context.worktree_id,
      write_scope: [...record.route_binding.write_scope], command_ids: [...this.qualityPolicy.command_ids],
      policy_hash: this.qualityPolicy.policy_hash, effective_policy_hash: record.effective_policy.policy_hash,
    ...
    await readEvidenceArtifact(this.store.root, report.diff_reference, report.diff_hash);
    ...
    const bundle = createEvidenceBundle({
      version: 3, bundle_id: `bundle-${stableHash({ run: record.execution_context.run_id, content: report.content_snapshot_hash, repair_count: record.repair_count, quality_report_hash: report.report_hash }).slice(0, 24)}`,
    ...
    const reference = await persistEvidenceBundle(this.store.root, bundle);
    if (beforeBundle) {
  ...
  private async loadCurrentBundle(record: CoreRecord): Promise<EvidenceBundle> {
    ...
    const bundle = JSON.parse(bytes.toString("utf8")) as unknown;
    assertEvidenceBundle(bundle);
    if (bundle.task_id !== record.task_package.task_id || !hashesEqual(bundle.bundle_hash, record.evidence_bundle_hash) || !hashesEqual(bundle.task_package_hash, record.task_package.task_package_hash) || !hashesEqual(bundle.route_binding_hash, record.route_binding.route_binding_hash) || !hashesEqual(bundle.policy_hash, record.effective_policy.policy_hash) || !hashesEqual(bundle.approval_hash, record.approval_record!.approval_hash)) throw new Error("EvidenceBundle is not bound to the unchanged approved contracts");
    return bundle;
  }
...
function approvalSummary(record: CoreRecord): RouterApprovalSummary {
  const base = {
    task_id: record.task_package.task_id, goal: record.task_package.goal,
    provider: record.route_binding.provider_id, model: record.route_binding.model_id,
    data_classification: record.task_package.data_classification,
    read_scope: [...record.route_binding.read_scope], write_scope: [...record.route_binding.write_scope],
    budget: { ...record.route_binding.request_budget }, task_package_hash: record.task_package.task_package_hash,
    route_binding_hash: record.route_binding.route_binding_hash, execution_context_hash: record.execution_context.execution_context_hash,
...
function assertCoreRecord(value: unknown): asserts value is CoreRecord {
  ...
  const keys = ["version", "task_package", "effective_policy", "route_binding", "execution_context", "worktree_binding", "approval_record", "execution_evidence", "evidence_bundle_hash", "evidence_bundle_reference", "quality_summary", "final_review", "review_history", "repair_count", "apply_record", "created_at", "updated_at"];
