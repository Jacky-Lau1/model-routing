...
export function createEvidenceBundle(input: EvidenceBundleInput): EvidenceBundle {
  ...
  const body: EvidenceBundleInput = {
    ...
    acceptance_results: Object.freeze({ ...input.acceptance_results, visible_tests: Object.freeze({ ...input.acceptance_results.visible_tests }), hidden_tests: Object.freeze({ ...input.acceptance_results.hidden_tests }) }),
    hidden_acceptance_result: input.hidden_acceptance_result === null ? null : Object.freeze({ ...input.hidden_acceptance_result, counts: Object.freeze({ ...input.hidden_acceptance_result.counts }), redaction_notes: Object.freeze([...input.hidden_acceptance_result.redaction_notes]) as unknown as string[] }),
...
export function assertEvidenceBundle(value: unknown): asserts value is EvidenceBundle {
  ...
  literal(object.version, 4, "EvidenceBundle.version");
...
function assertAcceptanceProjection(value: unknown, bundle: Record<string, unknown>): void {
  const item = exactObject(value, "EvidenceBundle.acceptance_results", ["version", "approval_boundary_hash", "quality_report_hash", "fixture_hash", "base_commit", "visible_tests", "hidden_tests", "regression", "scope_passed", "secret_passed", "diff_passed", "freeze_passed", "result_hash"]);
  literal(item.version, 1, "acceptance_results.version");
  for (const field of ["approval_boundary_hash", "quality_report_hash", "fixture_hash", "result_hash"] as const) hash(item[field], `acceptance_results.${field}`);
  commit(item.base_commit, "acceptance_results.base_commit");
  for (const field of ["regression", "scope_passed", "secret_passed", "diff_passed", "freeze_passed"] as const) if (typeof item[field] !== "boolean") throw new Error(`acceptance_results.${field} must be boolean`);
  for (const field of ["visible_tests", "hidden_tests"] as const) {
    const counts = exactObject(item[field], `acceptance_results.${field}`, ["passed", "failed", "not_run"]);
    for (const count of ["passed", "failed", "not_run"] as const) { integer(counts[count], `acceptance_results.${field}.${count}`, 0); if ((counts[count] as number) > 1_000_000) throw new Error(`acceptance_results.${field}.${count} exceeds its boundary`); }
  }
  const { result_hash: _hash, ...body } = item;
  if (!hashesEqual(item.result_hash as string, stableHash(body))) throw new Error("EvidenceBundle acceptance result hash changed");
  if (item.approval_boundary_hash !== bundle.quality_approval_boundary_hash || item.quality_report_hash !== bundle.quality_report_hash || item.fixture_hash !== bundle.fixture_hash || item.base_commit !== bundle.base_commit) throw new Error("EvidenceBundle acceptance results are not bound to quality/base evidence");
  const visible = item.visible_tests as { passed: number; failed: number; not_run: number }; const hidden = item.hidden_tests as { passed: number; failed: number; not_run: number };
  if ((visible.failed > 0 || visible.not_run > 0 || hidden.failed > 0 || hidden.not_run > 0 || !item.scope_passed || !item.secret_passed || !item.diff_passed || !item.freeze_passed) && item.regression !== true) throw new Error("EvidenceBundle acceptance regression contradicts deterministic gate evidence");
}

function assertHiddenAcceptanceProjection(value: unknown, bundle: Record<string, unknown>): void {
  if (value === null) return;
  const item = exactObject(value, "EvidenceBundle.hidden_acceptance_result", ["version", "command_id", "approval_boundary_hash", "fixture_hash", "base_commit", "counts", "passed", "exit_code", "timed_out", "output_overflowed", "hidden_root_unchanged", "diagnostic_hash", "output_summary", "redaction_notes", "result_hash"]);
  literal(item.version, 1, "hidden_acceptance_result.version"); literal(item.command_id, "project_acceptance", "hidden_acceptance_result.command_id");
  for (const field of ["approval_boundary_hash", "fixture_hash", "diagnostic_hash", "result_hash"] as const) hash(item[field], `hidden_acceptance_result.${field}`); commit(item.base_commit, "hidden_acceptance_result.base_commit");
  const counts = exactObject(item.counts, "hidden_acceptance_result.counts", ["passed", "failed", "not_run"]); for (const field of ["passed", "failed", "not_run"] as const) { integer(counts[field], `hidden_acceptance_result.counts.${field}`, 0); if ((counts[field] as number) > 1_000_000) throw new Error("Hidden acceptance count exceeds its boundary"); }
  for (const field of ["passed", "timed_out", "output_overflowed", "hidden_root_unchanged"] as const) if (typeof item[field] !== "boolean") throw new Error(`hidden_acceptance_result.${field} must be boolean`); if (!Number.isInteger(item.exit_code)) throw new Error("hidden_acceptance_result.exit_code must be integer");
  if (typeof item.output_summary !== "string" || Buffer.byteLength(item.output_summary) > 256 || !/^Hidden acceptance (?:passed|failed); detailed output remains local-only\.$/.test(item.output_summary)) throw new Error("Hidden acceptance summary is not bounded and opaque");
  if (!Array.isArray(item.redaction_notes) || item.redaction_notes.join("\0") !== "hidden_stdout_not_exported\0hidden_stderr_not_exported\0hidden_paths_not_exported\0reference_answer_not_exported") throw new Error("Hidden acceptance redaction evidence is invalid");
  const expected = (counts.passed as number) > 0 && counts.failed === 0 && counts.not_run === 0 && item.exit_code === 0 && item.timed_out === false && item.output_overflowed === false && item.hidden_root_unchanged === true; if (item.passed !== expected) throw new Error("Hidden acceptance passed flag contradicts evidence");
  const { result_hash: _hash, ...body } = item; if (!hashesEqual(item.result_hash as string, stableHash(body))) throw new Error("Hidden acceptance result hash changed");
  if (item.approval_boundary_hash !== bundle.quality_approval_boundary_hash || item.fixture_hash !== bundle.fixture_hash || item.base_commit !== bundle.base_commit) throw new Error("Hidden acceptance result is not bound to the EvidenceBundle");
}

function assertBundleQualityBindings(bundle: EvidenceBundle): void {
  const request = { run_id: bundle.run_id, task_id: bundle.task_id, base_commit: bundle.base_commit, plan_hash: bundle.task_package_hash, approval_hash: bundle.approval_hash, isolation_hash: bundle.isolation_hash, worktree_id: bundle.worktree_id, write_scope: [...bundle.quality_write_scope], command_ids: [...bundle.quality_command_ids], policy_hash: bundle.quality_policy_hash, catalog_hash: bundle.quality_catalog_hash, fixture_hash: bundle.fixture_hash, hidden_root_hash: bundle.hidden_root_hash, effective_policy_hash: bundle.policy_hash, max_wall_time_ms: bundle.quality_policy.max_wall_time_ms };
  ...
  const reportBody = { version: 1 as const, run_id: bundle.run_id, task_id: bundle.task_id, base_commit: bundle.base_commit, plan_hash: bundle.task_package_hash, approval_hash: bundle.approval_hash, isolation_hash: bundle.isolation_hash, worktree_id: bundle.worktree_id, policy_hash: bundle.quality_policy_hash, approval_boundary_hash: bundle.quality_approval_boundary_hash, catalog_hash: bundle.quality_catalog_hash, fixture_hash: bundle.fixture_hash, hidden_root_hash: bundle.hidden_root_hash, effective_policy_hash: bundle.policy_hash, max_wall_time_ms: bundle.quality_policy.max_wall_time_ms, request_hash: bundle.quality_request_hash, passed: bundle.quality_passed, worktree_head: bundle.worktree_head, files_changed: bundle.files_changed, content_snapshot_hash: bundle.content_snapshot_hash, post_artifact_snapshot_hash: bundle.post_artifact_snapshot_hash, worktree_snapshot_hash: bundle.worktree_snapshot_hash, diff_hash: bundle.diff_hash, diff_reference: bundle.diff_reference, quality_gate_results: bundle.quality_gate_results, tests_run: bundle.tests_run, scope_violations: bundle.scope_violations, privacy_violations: bundle.privacy_violations, secret_scan_summary: bundle.secret_scan_summary, wall_clock_time_ms: bundle.wall_clock_time_ms, redaction_notes: bundle.redaction_notes, hidden_acceptance_result: bundle.hidden_acceptance_result };
  if (!hashesEqual(bundle.quality_report_hash, stableHash(reportBody)) || !hashesEqual(bundle.acceptance_results.quality_report_hash, bundle.quality_report_hash)) throw new Error("EvidenceBundle quality report hash does not match its complete report projection");
  const outcomes = new Map(bundle.quality_gate_results.map(item => [item.gate_id, item.outcome]));
  const accepted = bundle.acceptance_results; const visibleCommands = bundle.quality_command_ids.filter(id => id !== "project_acceptance");
  const visible = visibleCommands.reduce((counts, id) => { const outcome = outcomes.get(id); if (outcome === "passed") counts.passed++; else if (outcome === "failed") counts.failed++; else counts.not_run++; return counts; }, { passed: 0, failed: 0, not_run: 0 });
  const hidden = bundle.hidden_acceptance_result?.counts ?? { passed: 0, failed: 0, not_run: 1 };
  const scopePassed = outcomes.get("preapply_scope") === "passed" && outcomes.get("changed_files_scope") === "passed"; const secretPassed = outcomes.get("secret_scan") === "passed"; const diffPassed = outcomes.get("diff_sanity") === "passed" && outcomes.get("evidence_artifact") === "passed"; const freezePassed = outcomes.get("final_freeze") === "passed" && bundle.content_snapshot_hash === bundle.post_artifact_snapshot_hash;
  if (stableHash(accepted.visible_tests) !== stableHash(visible) || stableHash(accepted.hidden_tests) !== stableHash(hidden) || accepted.scope_passed !== scopePassed || accepted.secret_passed !== secretPassed || accepted.diff_passed !== diffPassed || accepted.freeze_passed !== freezePassed) throw new Error("EvidenceBundle acceptance projection contradicts gate, hidden, or freeze evidence");
