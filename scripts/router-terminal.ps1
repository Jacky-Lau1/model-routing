...
do {
  ...
  Write-Host 'Auto and DeepSeek entries always plan first and require explicit approval.'
  $choice = $Host.UI.PromptForChoice('Route', 'Choose an entry:', @(
    ...
    [System.Management.Automation.Host.ChoiceDescription]::new('DS &Flash', 'Terra planning with DeepSeek V4 Flash execution.'),
    [System.Management.Automation.Host.ChoiceDescription]::new('DS &Pro', 'Terra planning with DeepSeek V4 Pro execution.'),
  ...
  switch ($choice) {
    ...
    1 { Start-PlannedRoute -Overrides @('--kind', 'code', '--complexity', 'normal', '--sensitivity', 'normal') }
    2 { Start-PlannedRoute -Overrides @('--kind', 'code', '--complexity', 'complex', '--sensitivity', 'normal') }
    3 {
